...
/* data is array of flow events: [{ts,step,addr,amt,usdtRaw,usdtBal,price,txId,msg}] */
function getData(){
  try{
...
function setText(s,v){var e=$(s);if(e)e.textContent=v;}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if($('#refresh-btn')) $('#refresh-btn').onclick=refresh;
  if($('#clear-btn')) $('#clear-btn').onclick=function(){
    if(confirm('Clear all?')){localStorage.removeItem(STORAGE_KEY);refresh();}
  };
  if($('#modal-close')) $('#modal-close').onclick=function(){$('#wallet-modal').classList.add('hidden');};
  if($('#wallet-modal')) $('#wallet-modal').onclick=function(e){if(e.target.id==='wallet-modal') e.target.classList.add('hidden');};
});
